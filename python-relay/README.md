# TunnelDesktop Python Relay

This is a Python/FastAPI implementation of the TunnelDesktop Azure WebSocket relay.
...
- WebSocket roles through `X-TunnelDesktop-Role`: `agent`, `client`, `home-agent`, `probe`, and `dashboard`.
...
```text
tunneldesktop-relay.service
```

Service layout:

```text
/opt/tunneldesktop/python-relay/app.py
/opt/tunneldesktop/python-relay/vendor/
/etc/systemd/system/tunneldesktop-relay.service
```

The systemd service uses:

```text
PYTHONPATH=/opt/tunneldesktop/python-relay/vendor
...
```sh
systemctl status tunneldesktop-relay
